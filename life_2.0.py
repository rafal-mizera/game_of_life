    ...
    for x in range(cells_w):
        for y in range(cells_h):
            if game_field[x][y] == alive:
                pygame.draw.rect(game_window, (col1, col2, col3),
                                 Rect((x * cell_size, y * cell_size), (cell_size, cell_size)), 1)

def count_alive(gamefield):
    cells_alive = 0
    for i in range(cells_h):
        for j in range(cells_w):
            if gamefield[j][i] == alive:
                cells_alive += 1
    return cells_alive
...
while running:
    ...
    if life_goes is True:
            if count_alive(game_field) > 0:
                game_field = prepare_population(game_field)
            else:
                image = pygame.image.load('end.jpg')
